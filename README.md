...
- **日志记录**：捕获 `log` 工具输出的 info/warn/error 级别日志，自动格式化 JSON 内容，同时在 Node 部署时会把同样的日志写入 `data/logs/*.log`，方便管理端查看或下载。
...
| LOG_RETENTION_DAYS | 【可选】日志落盘后的保留天数，单位天，默认为`7`，设置为`0`或负数可关闭自动清理，日志文件会写入项目根目录的 `data/logs` 文件夹       |
